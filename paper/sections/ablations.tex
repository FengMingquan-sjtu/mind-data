...
To evaluate the impact of prompt styles, we conduct experiment by training with conversations from seven different styles. From \autoref{tab:7b_owm_4b}, we observe improvement across all tasks using six conversational styles. However, our experiment with \tp conversations yield relatively equivalent or worse performance compared to the raw data (\autoref{tab:7b_tp}). 
...
by replacing \owma with \mathpile and \textsc{mind}-\mathpile accordingly. \autoref{fig:mp_exp} shows that \textsc{mind}-\mathpile outperforms the raw counterpart in all three math benchmarks along with specialized knowledge tasks, achieving comparable scores in general reasoning task. In addition, majority of \mathpile data is from ArXiV papers and recent work has found this source ineffective in improving mathematical reasoning \citep{deepseek-math}. We observe the similar trend with \gsm benchmark where training with non-math focused pretraining corpora yields better score than with \mathpile corpus. However, our synthetic conversation on \mathpile rather amplifies the quality of the corpus resulting in 3.95\% %and 2.96\% 
...
To assess the significance of \ourapproach over other synthetic data generation approach for math reasoning, we investigate %Web Rephrase Augmented Pretraining (WRAP), introduced by \cite{rephrasing-the-web}. WRAP proposes 
%a way to upsample high-quality data by 
rephrasing the raw web text with \llm, introduced by \cite{rephrasing-the-web}. They generate four styles of synthetic data (easy, medium, hard, and Q/A) among which medium style rephrasing produces improvements across many tasks. %In addition, they found maximum gain by combining raw and rephrased data. 
To maintain consistency among generation quality and training setup, we choose the \owma-4B data and generate rephrases with $\mathcal{M}$ using the highest performing prompt from the paper. We %maintain the same data blend as before and simply replace \owma-4B with Rephrase-\owma-4B or combine both by sampling raw and rephrase data in a 1:1 ratio. Following the previous experimental setup, we take the 7B base model from 700B checkpoint and continuously train with 50B tokens using the new data blend.
continuously train $\mathcal{C}$ with $\mathcal{D} = \{\mathcal{X} \cup \mathcal{R}_{pt}\}.$ where $\mathcal{X} \in $ \{Rephrase-\owma-4B\}.%, \owma-4B $\cup$ Rephrase-\owma-4B\}.
%\syeda{Do we want a table with all conversation styles vs rephrase or a chart?}

% \begin{figure}[H]
%   \centering
%   \includesvg[width=0.8\columnwidth]{figures/rephrase.svg}
%   \caption{\textbf{Rephrase vs Conversations.} Continuous training of 7B \llm separately with conversational synthetic data and rephrase shows that conversation is better for mathematical and general reasoning than simple rephrasing.}
%   \label{fig:rephrase}
% \end{figure}

% OWM	12.96	4.92	45.91	39.39	52.90	31.22
% Rephrase-OWM	11.68	5.46	46.17	39.71	53.58	31.32
% Rephrase-OWM+OWM [1:1]	14.25	6.20	48.74	42.31	53.68	33.04
% Longest Conversation	25.78	6.30	49.37	42.72	54.86	35.81
% Conv-Math-Data+OWM [1:1]	21.68	6.14	49.57	42.56	54.50	34.89
...
Our findings in \autoref{tab:7b_owm} and \autoref{tab:7b_owm_4b} indicate that completely replacing \owm with synthetic data provides the best performance across benchmarks. However, %prior work 
\cite{rephrasing-the-web} emphasizes the importance of combining real data and synthetic rephrases to achieve consistent improvements across a broader range of tasks---a similar trend we observe in our experiment with rephrased data, as shown in \autoref{tab:7b_comp_data}. To investigate this further, we conduct experiments with four different data combinations using \owma-4B while the $\mathcal{R}_{pt}$ remains constant:
